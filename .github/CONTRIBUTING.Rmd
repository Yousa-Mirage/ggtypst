...
- [`Air`](https://posit-dev.github.io/air/) for formatting R code and [`Jarl`](https://jarl.etiennebacher.com/) for checking R code
...
- Optional: [`pre-commit`](https://pre-commit.com/) or [`prek`](https://prek.j178.dev/) to run some checks on staged files before committing
...
`src/rust/`, then links that library into the R package through [`extendr`](https://extendr.rs/).
...
- `zzz.R`: performs runtime registration that cannot be expressed statically,
...
The three embedded Typst files under `src/rust/specs/`, which are from MiTeX's repository,
provide the MiTeX scope used when evaluating LaTeX-style formulas in Typst.
...
If you are unsure how to begin, you are welcome to open an issue or draft PR
